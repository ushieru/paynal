...
exports.CLIENT_FRAMES = {
    CONNECT: (login, passcode) => {
        ...
        return new core_1.Frame('CONNECT', headers);
    },
    STOMP: (login, passcode) => {
        ...
        return new core_1.Frame('STOMP', headers);
    },
    SEND: (destination, headers, body) => {
        headers.destination = destination;
        if (body)
            headers['content-length'] = body.length;
        return new core_1.Frame('SEND', headers, body);
    },
    SUBSCRIBE: (destination, headers) => {
         ...
         */
        // headers.ack = 'client'
        return new core_1.Frame('SUBSCRIBE', headers);
    },
    UNSUBSCRIBE: (id) => {
        return new core_1.Frame('UNSUBSCRIBE', { id });
    },
    BEGIN: (transaction, headers) => {
        headers.transaction = transaction;
        return new core_1.Frame('BEGIN', headers);
    },
    COMMIT: (transaction, headers) => {
        headers.transaction = transaction;
        return new core_1.Frame('COMMIT', headers);
    },
    ABORT: (transaction, headers) => {
        headers.transaction = transaction;
        return new core_1.Frame('ABORT', {});
    },
    ACK: (id, headers, transaction) => {
        headers.id = id;
        if (transaction)
            headers.transaction = transaction;
        return new core_1.Frame('ACK', headers);
    },
    NACK: (id, headers, transaction) => {
        headers.id = id;
        if (transaction)
            headers.transaction = transaction;
        return new core_1.Frame('NACK', headers);
    },
    DISCONNECT: (receipt) => {
        return new core_1.Frame('DISCONNECT', { receipt });
    },
};
