...
import { Break } from './bytes'
import { Frame } from "./frame"
import { SERVER_FRAMES } from './frames'
...
export class Server extends EventEmitter {
    ...
    sendClient(
        ...
        const middleware = this.withMiddleware('send', (_socket, _args, callback) => {
            this.subscribers.forEach(subscriber => {
                ...
                if (!checkSubMatchDestination(subscriber, topic)) return
                if (!subscriber.socket) return
                const messageFrame = SERVER_FRAMES.MESSAGE(subscriber.id, topic, frame.headers, frame.body)
    ...
    subscribeClient(
        ...
        const middleware = this.withMiddleware('subscribe',
            (socket, { dest, id }: { dest: string, id: string }) => {
                const sub: Subscriber = {
                    id: id,
                    sessionId: socket.sessionId,
                    topic: dest,
                    tokens: tokenizeDestination(dest),
                    socket: socket
                }
                this.subscribers.push(sub)
                this.emit('subscribe', sub)
                this.config.debug('Server subscribe', id, dest)
            })
        middleware(socket, args)
    }
